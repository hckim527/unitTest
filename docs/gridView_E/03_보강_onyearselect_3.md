# gridView_E_onyearselect_3.xml — 보강 (API 가이드 1:1 대조)

- 샘플 경로 : `unitTest/src/main/webapp/sample/gridView/gridView_E_onyearselect_3.xml`
...
| `@description` 첫 문장 : inputType="calendar" 인 Body Cell 의 팝업 달력에서 연도의 선택을 변경했을때 발생합니다. | description 필드 (`_1` 과 **동일 문구 유지**) | ✓ (validation 아님) |
| `@description` 하위절 : 팝업 달력의 상단 SelectBox 에 의해 연도가 변경된 경우만 발생합니다. | - | 제외 (`_1` 담당) |
...
| `@param info` (변경된 연도 정보 객체) | - | 제외 (`_1` 담당) |
| `@param info.oldValue` (변경 전 연도, yyyy 숫자) | - | 제외 (`_1` 담당) |
| `@param info.newValue` (변경 후 연도, yyyy 숫자) | - | 제외 (`_1` 담당) |
| `@spec` 1 (이전달/이후달/이전연월/다음연월 버튼 시 미발생) | - | 제외 (`_1` 담당) |
| `@spec` 2 (onyearselect -> onmonthselect -> ondateselect 순서) | - | 제외 (`_1` 담당) |
| `@return` | (없음) | 해당 없음 |
| `@related` ondateselect / onmonthselect / onviewchange / column.inputType | - | 제외 (`@related` 는 검증 대상 아님, UT_01 §2) |

→ **담당 범위 누락 0건.** 나머지는 의도적 중복 제외.

---

## 2. 구성 방식 (짝 샘플 `onmonthselect_3` 과 동일 구성, 조작만 연도로 변경)
...
- **실행일 비의존 설계** : 팝업 달력은 Cell 에 저장된 날짜의 연/월로 열린다. 개발본부 Cell 은 `20260210` 고정이고,
  연도 SelectBox 만 조작하고 **날짜는 선택하지 않으므로 Cell 값이 변하지 않는다**
  → 팝업은 **매번 2026년**으로 열리고 `info.oldValue` 는 항상 `2026`. 시스템 현재 날짜 하드코딩·오늘 버튼 불필요.
...
- 핸들러는 rowIndex 검증 전용이므로 **rowIndex 를 개별 출력**하고, 이벤트가 연도 SelectBox 로 발생했음을 확인할 수 있도록
  헤더 줄에 선택 Row 이름 + `연도 oldValue -> newValue` 를 함께 표시. 같은 Row 에서 다시 변경하면
...
## 3. MCP 실측 raw (playwright2, port 62358)
...
달력/연 SelectBox 조작 DOM (전역 오버레이 — 컬럼 id 로 좁힐 것)
- 편집 진입 : `td#mf_target1_cell_{r}_{c}` 클릭 (embeddedInput 없음)
- 팝업 오픈 버튼 : `#G_mf_target1__cdate_img`
- **연 SelectBox** : `#mf_G_mf_target1__cdate_calendar_selectbox_year(_label)` (WebSquare selectbox DIV)
  - 항목 : `#mf_G_mf_target1__cdate_calendar_selectbox_year_itemTable_{n}` — `_label` 클릭 후 **지연 생성**
  - native `<select>` 아님 → `select_option` 사용 불가
  - 실측 선택 가능 연도 : `1978년 ~ 2030년` + `9999년` (총 54개, 인덱스 0=1978년 … 52=2030년, 53=9999년)
    → **연도 항목 인덱스는 선택 가능 범위에 좌우되므로 텍스트(`/^2027년$/`)로 로케이팅**할 것
- 팝업 닫기 : `Escape` 1회(팝업) / 2회(편집기). **연 변경은 팝업을 닫지 않음**

토글 아이콘 : `td#mf_target1_cell_0_0 span.w2grid_minus` (접힘 시 `span.w2grid_plus`)
- 실측 : `class="w2grid_minus w2grid_root" data-openstatus="1"` (접으면 `w2grid_plus` / `0`)
- **토글 클릭은 Row 선택을 유발하지 않음** (이벤트 출력에 영향 없음)

접은 뒤 표시 셀 (stale `td` 주의)
```
0 영업본부 / 1 개발본부 / 2 개발1팀 / 3 관리팀
+ tr.w2grid_hidedRow 로 숨겨진 Row 가 텍스트째 잔존 → DOM 텍스트 판정 금지, 이벤트 출력으로만 판정
```

결과창 출력 (순서대로) :
```
▶ onyearselect (선택 Row : 개발본부, 연도 2026 -> 2027)
rowIndex : 3
▶ onyearselect (선택 Row : 개발본부, 연도 2026 -> 2028)
rowIndex : 1
[개발본부] 이전 변경 rowIndex : 3 / 현재 rowIndex : 1
▶ onyearselect (선택 Row : 개발본부, 연도 2026 -> 2029)
rowIndex : 3
...
**가이드 문구와 일치(엔진 결함 없음).**
`info.oldValue` 는 3회 모두 `2026` (Cell 값 `2026-02-10` 불변) → **실행일과 무관한 고정 기대값**.

부수 확인
- 접힘 상태에서도 `getRealRowIndex(표시인덱스)` 정상 (표시1 → DataList3 = 개발본부).
- `rowNumVisible="true"` 의 rowNum 컬럼은 Body Column 인덱스에 영향 없음.
- console error 0 / label1 1개 / createLabel 은 XML 상 1줄(개행 이스케이프만) 유지.

---

## 4. 엔진 소스 근거

`websquare/uiplugin/gridView/cellController.js:1960~1970` (편집 진입 방식 = `viewType="icon"` 경로)
```js
name: "onyearselect",
...
...
```
`cellInfo.editRowIndex` 는 `setEditMode(td, rowIndex)` 에서 설정되는 **표시(display) 기준 Row 인덱스**이며,
onmonthselect / ondateselect 와 완전히 동일한 경로다.
